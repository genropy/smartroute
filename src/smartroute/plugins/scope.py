...
Responsibilities
----------------
- allow handlers to declare arbitrary scope tags (e.g. INTERNAL, PUBLIC_SHOP)
- resolve which uppercase channel codes (CLI, SYS_HTTP, HTTP, ...) may expose
  each scope, producing metadata consumed by Publisher/CLI
- honour per-router defaults (`.plug("scope", scopes=..., channels=...)`) and
  fine-grained overrides via `configure()`

Guarantees
----------
- channel codes are always uppercase symbolic tokens; invalid input raises
  `ValueError`
- introspection (`Router.describe/members`) surfaces the computed scope map so
  external processes can filter routes without recomputing logic
"""
