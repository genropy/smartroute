...
**3. ScopePlugin** - Visibility management
<!-- test: test_scope_plugin.py::test_scope_plugin_describe_and_channel_matrix -->

```python
router.plug("scope", scopes="internal",
            scope_channels={"internal": ["CLI", "SYS_HTTP"]})
...
router.plug("scope", scope_channels={"internal": ["CLI"]})
...
### When should I use ScopePlugin?

**Question**: In which scenarios is ScopePlugin useful?

**Answer**: Use `ScopePlugin` when:
...
# Configuration
router.scope.set_config(scope_channels={
    "admin": ["CLI"],
    "public": ["CLI", "HTTP"]
})
```
